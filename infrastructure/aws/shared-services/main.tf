...
locals {
  ...
  # IAM
  assume_log_role_policy = "arn:aws:iam::245145356204:policy/dev-assume-log-role-policy"
...
################################################################
## Attach Assume Role Policy for Shared Services Logging Role ##
################################################################

resource "aws_iam_role_policy_attachment" "assume_log_role_attachment" {
  role       = module.k8s_prometheus.k8s_wk_role_name
  policy_arn = local.assume_log_role_policy
}
...
resource "aws_security_group_rule" "allow_nodeport" {
  ...
  cidr_blocks = [
    local.vpc_cidr_block,          #Shared Service Dev
