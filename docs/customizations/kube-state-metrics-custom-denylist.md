...
The example above makes use of a [feature] introduced in the v2.16.0 release of KSM, which allows users to utilize lookarounds within regular expressions. As such, it keeps almost all of the default deny-list, but filters out only the `kube_namespace_created` metric allowing that to be generated.

[feature]: https://github.com/kubernetes/kube-state-metrics/pull/2616
